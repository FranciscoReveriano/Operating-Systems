...
int main(int argc, char *argv[]) {
	...
	if (result.error == 1) {
		...
		printf("\nMemory Blocks Used : %d", result.MemoryBlocksUsed);
		printf("\nMemory Final Block Address: %d in Frame: %d", result.FinalBlockAddress, result.MemoryBlocksUsed);
		printf("\nMemory Blocks Available: %d", result.MemoryBlocksRemaining);
